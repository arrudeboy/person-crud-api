...
public class IDEPathHelper {
    ...
    static {
        try {
            var projectRootDir = Paths.get(requireNonNull(
                    IDEPathHelper.class.getResource("gatling.conf"),
                    "Couldn't locate gatling.conf").toURI()).getParent().getParent().getParent();
            var mavenTargetDirectory = projectRootDir.resolve("target");
            var mavenSrcTestDirectory = projectRootDir.resolve("src").resolve("test");
